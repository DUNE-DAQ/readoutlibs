...
struct KeyGetter
{
  uint32_t operator()(types::VariableSizePayloadTypeAdapter& wrapper) // NOLINT(build/unsigned)
  {
...
BOOST_AUTO_TEST_CASE(VariableSizeElementQueue_find)
{
  ...
  SearchableProducerConsumerQueue<types::VariableSizePayloadTypeAdapter, uint32_t, KeyGetter> // NOLINT(build/unsigned)
    queue(10000);
  for (uint32_t timestamp_counter = 0; timestamp_counter < 10000; timestamp_counter += 10) { // NOLINT(build/unsigned)
    ...
    types::VariableSizePayloadTypeAdapter wrapper(1024, payload);
    ...
    for (uint32_t expected_timestamp = 0; expected_timestamp <= timestamp_counter; // NOLINT(build/unsigned)
      ...
      uint32_t timestamp; // NOLINT(build/unsigned)
      types::VariableSizePayloadTypeAdapter* found_element = queue.find_element(expected_timestamp);
...
BOOST_AUTO_TEST_CASE(VariableSizeElementQueue_find_outside)
{
  ...
  types::VariableSizePayloadTypeAdapter* found_element = queue.find_element(100);
...
BOOST_AUTO_TEST_CASE(VariableSizeElementQueue_overrun)
{
  ...
  for (int i = 0; i < 500; ++i) {
    types::VariableSizePayloadTypeAdapter wrapper;
