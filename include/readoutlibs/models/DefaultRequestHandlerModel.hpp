    ...
    while (m_run_marker.load() || m_waiting_requests.size() > 0) {
      {
        ...
        for (size_t i = 0; i < size;) {
          if (m_waiting_requests[i].request.request_information.window_end < newest_ts) {
            issue_request(m_waiting_requests[i].request, *(m_waiting_requests[i].fragment_sink), false);
            ...
            issue_request(m_waiting_requests[i].request, *(m_waiting_requests[i].fragment_sink), true);
